...
bool loadMedia()
{
	...
	if( !gWaitingTexture.loadFromFile( "pics/waiting.png" ) )
	{
	
		printf( "Failed to load front waiting texture!\n" );
		success = false;
	}
	else
	{
		//Set standard alpha blending
		gWaitingTexture.setBlendMode( SDL_BLENDMODE_BLEND );
	}
	...
	if( !gPlayerAwinsTexture.loadFromFile( "pics/Fwins.png" ) )
	{
		printf( "Failed to load front pause texture!\n" );
		success = false;
	}
	...
	if( !gPlayerBwinsTexture.loadFromFile( "pics/Pwins.png" ) )
	{
		printf( "Failed to load front pause texture!\n" );
		success = false;
	}
	...
	else
	{
		...
		{	
			...
			while( !quit )
			{
				...
				timeText.str( "" );
				timeText << "Time left: " << (120000-(SDL_GetTicks() - startTime))/60000<<"min "<<(((120000-(SDL_GetTicks() - startTime))/1000)%60) <<"sec"; 
