...
bool loadMedia()
{
	...
	gHigh = Mix_LoadWAV( "smb_coin.wav" );
	if( gHigh == NULL )
	{
	...
	gEnergy = Mix_LoadWAV( "smb_powerup.wav" );
	if( gEnergy == NULL )
	{
		printf( "Failed to load high sound effect! SDL_mixer Error: %s\n", Mix_GetError() );
		success = false;
	}
	gPause =Mix_LoadWAV( "smb_pause.wav" );
	if( gPause == NULL )
	{
		printf( "Failed to load high sound effect! SDL_mixer Error: %s\n", Mix_GetError() );
		success = false;
	...
	gMedium = Mix_LoadWAV( "smb_mariodie.wav" );
	if( gMedium == NULL )
	{
	...
	gLow = Mix_LoadWAV( "smb_flagpole.wav" );
	if( gLow == NULL )
	{
	...
	if( !gInstrWindowTexture.loadFromFile( "instructions.png" ) )
	{
